import { pgTable, index, uniqueIndex, foreignKey, unique, pgPolicy, text, uuid, integer, boolean, timestamp, bigint, primaryKey } from "drizzle-orm/pg-core"
...
export const users = pgTable("users", {
	...
	uniqueIndex("users_discord_id_uidx").using("btree", table.discordId.asc().nullsLast().op("text_ops")).where(sql`(discord_id IS NOT NULL)`),
	foreignKey({
	...
	pgPolicy("authenticated user can read own row", { as: "permissive", for: "select", to: ["authenticated"], using: sql`(auth_user_id = auth.uid())` }),
]);
...
export const members = pgTable("members", {
	...
	pgPolicy("admin user can read all rows", { as: "permissive", for: "select", to: ["authenticated"], using: sql`(((auth.jwt() -> 'app_metadata'::text) ->> 'role'::text) = 'admin'::text)` }),
...
export const channels = pgTable("channels", {
	...
	index("idx_channels_category_id").using("btree", table.categoryId.asc().nullsLast().op("text_ops")),
	foreignKey({
...
export const grades = pgTable("grades", {
	...
	pgPolicy("authenticated user can read public.grades", { as: "permissive", for: "select", to: ["authenticated"], using: sql`true` }),
]);
...
export const userRole = pgTable("user_role", {
	...
	foreignKey({
			...
			foreignColumns: [users.discordUserId],
			name: "user_roles_discord_user_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.roleId],
			foreignColumns: [roles.roleId],
	...
	primaryKey({ columns: [table.discordUserId, table.roleId], name: "user_roles_pkey"}),
]);
