...
export const UserSchema = z.object({
    ...
    auth_user_id: z.uuid().nullable().openapi({ example: "auth0|1234567890abcdef" }),
    discord_id: z.string().nullable().openapi({ example: "123456789012345678" }),
    member_id: z.uuid().nullable().openapi({ example: "member-123" }),
}).openapi("User")

export const UpdateUserSchema = z.object({
    display_name: z.string().optional().openapi({ example: "John Doe" }),
    auth_user_id: z.uuid().nullable().optional().openapi({ example: "auth0|1234567890abcdef" }),
    discord_id: z.string().nullable().optional().openapi({ example: "123456789012345678" }),
    member_id: z.uuid().nullable().optional().openapi({ example: "member-123" }),
