...
class TestChannelCreate:
    ...
    def test_create_channel_with_all_fields(self, client):
        response = client.post(
            ...
            json={"name": "test-channel", "category_id": 400, "position": 3},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["name"] == "test-channel"
        assert data["category_id"] == 400
...
class TestChannelDelete:
    ...
    def test_delete_channel_missing_id(self, client):
        response = client.post(
            "/api/v0/channel/delete",
            json={},
        )
...
class TestChannelListRole:
    ...
    def test_list_channel_roles(self, client):
        response = client.get("/api/v0/channel/list-role", params={"channel_id": 300})
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
    ...
    def test_list_channel_roles_missing_channel_id(self, client):
