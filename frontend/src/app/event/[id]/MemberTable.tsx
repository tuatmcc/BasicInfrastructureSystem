...
const customFilterFn: FilterFn<ReactionMemberRow> = (row, columnId, filterValue: { operator: string, value: string }) => {
...
const buildCsv = (rows: ReactionMemberRow[]) => {
  const headers = [
    '名前',
    '学年',
    '学籍番号',
...
export default function MemberTable({ eventTitle, data }: MemberTableProps) {
  ...
  const handleExport = () => {
    setIsExporting(true);
    try {
      const csvContent = buildCsv(visibleRows);
